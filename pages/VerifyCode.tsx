                ...
                <form onSubmit={handleProceed}>
                    <div className="flex justify-center gap-4 mb-8">
                        {otp.map((digit, index) => (
                            <input
                                key={index}
                                // FIX: The ref callback was implicitly returning a value, which caused a TypeScript type error.
                                // By using a block body (`{...}`), we ensure the arrow function returns `void`, which satisfies the type expected by the `ref` prop.
